...
# PipeLine类
class PipeLine:
    def __init__(self,rgb888p_size=[224,224],display_mode="hdmi",display_size=None,osd_layer_num=1,debug_mode=0,line_rgb565_size=None):
        # sensor给AI的图像分辨率
        ...
        self.line_rgb565_size=line_rgb565_size
        self.line_channel=None
        self.line_status="disabled"
    ...
    def create(self,sensor=None,sensor_id=None,hmirror=None,vflip=None,fps=60,to_ide=True,crop_vertical=False):
        with ScopedTiming("init PipeLine",self.debug_mode > 0):
            ...
            if crop_vertical:
                ...
                self._config_line_channel(crop=(crop_x,crop_y,crop_w,crop_h))
            else:
                # 通道0直接给到显示VO，格式为YUV420
    ...
    def _config_line_channel(self,crop=None):
        if self.line_rgb565_size is None:
            self.line_status="disabled"
            return
        try:
            size=[ALIGN_UP(self.line_rgb565_size[0],16),self.line_rgb565_size[1]]
            if crop is None:
                self.sensor.set_framesize(w=size[0], h=size[1], chn=CAM_CHN_ID_1)
            else:
                self.sensor.set_framesize(w=size[0], h=size[1], chn=CAM_CHN_ID_1, crop=crop)
            self.sensor.set_pixformat(Sensor.RGB565, chn=CAM_CHN_ID_1)
            self.line_channel=CAM_CHN_ID_1
            self.line_status="ok"
            print("PipeLine line RGB565 channel:", size)
        except Exception as e:
            self.line_channel=None
            self.line_status="config-failed"
            print("PipeLine line RGB565 channel disabled:", e)

    def get_line_frame(self):
        if self.line_channel is None:
            self.line_status="no-channel"
            return None
        try:
            self.line_frame = self.sensor.snapshot(chn=self.line_channel)
            self.line_status="ok"
            return self.line_frame
        except Exception as e:
            print("PipeLine get_line_frame failed:", e)
            self.line_channel=None
            self.line_status="snapshot-failed"
            return None
